  ...
  describe('Constructor:', () => {
    ...
    it('Constructor throws errors given valid Firestore QuerySnapshot and an invalid center', (done) => {
      stubDatabase()
        ...
        .then((snapshot) => {
          // @ts-ignore
          expect(() => new GeoQuerySnapshot(snapshot, {})).to.throw();
        })
        .then(done);
    });
  });
      ...
      stubDatabase()
        ...
        .then((snapshot) => {
          ...
          expect(results).to.have.deep.members([{
            exists: true, id: 'loc1',
            data: { key: 'loc1', coordinates: new firebase.firestore.GeoPoint(2, 3), count: 0 },
            distance: null
          }, {
            exists: true, id: 'loc2',
            data: { key: 'loc2', coordinates: new firebase.firestore.GeoPoint(50, -7), count: 1 },
            distance: null
          }, {
            exists: true, id: 'loc3',
            data: { key: 'loc3', coordinates: new firebase.firestore.GeoPoint(16, -150), count: 2 },
            distance: null
          }, {
            exists: true, id: 'loc4',
            data: { key: 'loc4', coordinates: new firebase.firestore.GeoPoint(5, 5), count: 3 },
            distance: null
          }, {
            exists: true, id: 'loc5',
            data: { key: 'loc5', coordinates: new firebase.firestore.GeoPoint(67, 55), count: 4 },
            distance: null
          }, {
            exists: true, id: 'loc6',
            data: { key: 'loc6', coordinates: new firebase.firestore.GeoPoint(8, 8), count: 5 },
            distance: null
          }]);
        })
        .then(done);
  ...
  describe('forEach():', () => {
    it('forEach() enumerates all of the documents in the GeoQuerySnapshot', (done) => {
      stubDatabase()
        .then(() => collection.get())
        .then((snapshot) => {
          (new GeoQuerySnapshot(snapshot)).forEach((result) => expect(result).to.not.equal(null));
        })
        .then(done);
    });
  });
